...
from KLRG import KLRG


class OneVsAll(object):
    def __init__(self, num_labels=16, test_size=0.2, sigma=3, rank=55):
    ...
    def calc_z(self,KLRG_model,X_train,X_test,y_train,KernelMat):
        m, n = X_test.shape
        #不切分数据集的时候这里用KernelMat
        kernelMat = self.kernelMat(X_train, X_test)
        all_Zs = np.zeros((m, self.num_labels))  # 初始化每一列对应相应分类的z
        for i in range(self.num_labels):
            print("训练第{}个分类器".format(i))
            alphas, b = KLRG_model.calcNewAlpha(X_train, y_train[:, i], KernelMat)
            z = np.dot((alphas * y_train[:, i]), KernelMat) - b #带入包含测试集的kernel
...
if __name__ == '__main__':
    ...
    # 近似计算核矩阵
    KernelMat = model.K(X_train, model.rank)
    ...
    rank = model.rank
    #保存all_z
    np.savetxt("/home/donjy/datasets/gen_datas/pro_SMO_indian_{}_0.2_{}.csv".format(sigma,rank), all_z[:, :], delimiter=',')
    # '''
    #读取保存的z
    # all_z = pd.read_csv(r"/home/donjy/datasets/gen_datas/SMO_indian_3_0.2.csv", delimiter=",", header=None).values
