...
class SMLRFista(object):
    ...
    def MKL(self, X_train, y_train):
        ...
        from mklaren.kernel.kinterface import Kinterface
        K_exp = Kinterface(data=X_train, kernel=rbf_kernel, kernel_args={"sigma": 3})  # RBF kernel
        K_exp2 = Kinterface(data=X_train, kernel=rbf_kernel, kernel_args={"sigma": 2})  # RBF kernel
        K_exp3 = Kinterface(data=X_train, kernel=rbf_kernel, kernel_args={"sigma": 4})  # RBF kernel
...
if __name__ == '__main__':
    ...
    smlr = SMLRFista(
        ...
        t=1e-0, lamda=1e-3, max_iter=3000, alpha=0.5 ,sigma=3   #acc:82.17  sigma=3,lamda=1e-4
    )
    start = time.time()
    smlr.minimize()
    end = time.time()
